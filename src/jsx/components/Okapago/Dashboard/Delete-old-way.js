        ...
        <div className="basic-form">
            <div className="row">
                <div className="col text-center">
                    ...
                    <br/>
                    Advertencia <br/>
                    Esta Orden sera dada de baja <br/>a
...
function AccountResponse(props) {
    ...
    let result = 'Exitoso'
    let description = 'Su Orden ha sido eliminada'
    let symbol = true;
        ...
        <div className="card-body" >
            {props.loadingStatus ?
                <div className={"row justify-content-center"}>
                    <p>
                        Procesando su solicitud de eliminar Orden
                    </p>
...
const Delete= (props) => {
    ...
    const postData = {orderId: props.orderId};

    //adding account
    function modalVerify() {
        setModalData(false)
        setDataResult(true)
        deleteOrder().then(r => console.log('ejecutado DeleteRequest'))
    }

    //account save endpoint
    const  filteredData = 'okData'

    async function deleteOrder(response) {
        await axios.post(`/orders/delete`, postData,).then( async (res) => {
  ...
  const color = "#1a5a6e"
    ...
    return (
        <div className="bootstrap-modal" >
            ...
            <button onClick={() => openModal()}  data-bn-type="button" className=" btnSmall fc-Red fs-12">{props.children}</button>
            {/* <!-- Modal --> */}
            <Modal className="fade" show={basicModal} onHide={setBasicModal}>
                <Modal.Header className={"text-center justify-content-center"}>
                    <h4 className="text-black">Eliminar Orden</h4>
                    <Button
                        variant=""
                        className="close"
                        onClick={() => closeModal()}
